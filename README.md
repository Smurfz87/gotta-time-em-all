...
**Interval mode** *(in progress)* — Participants are split into pace groups, each with a rolling countdown (send-off). Tap a participant when they finish each rep. Overflow behaviour when someone misses their send-off is configurable: rejoin the group, get a fresh send-off, or get a fresh send-off with extra rest.

**Rest mode** *(in progress)* — All participants start together. Tap each participant when they finish a rep — a personal rest countdown begins, then their next rep starts automatically.
